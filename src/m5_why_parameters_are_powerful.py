...
def main():
    ...
    even_better_draw_circles(rg.Point(100,50),10,15,'black',2)
    even_better_draw_circles(rg.Point(-200,0),5,20,'purple',4)
    window.update()
...
def even_better_draw_circles(point,radius,number,color,thickness):
    purple_haze=rg.SimpleTurtle()
    purple_haze.pen=rg.Pen(color, thickness)
    purple_haze.pen_up()
    purple_haze.go_to(point)
    purple_haze.set_heading(0)  # Point "east" (towards the right)
    ...
    for k in range(1, number):  # k becomes 1, 2, 3, ... 10
        ...
        # Go DOWN 15 pixels, ending up pointing east again
        purple_haze.right(90)
        purple_haze.forward(15)
        purple_haze.left(90)

        purple_haze.pen_down()
        purple_haze.draw_circle(radius * k)
